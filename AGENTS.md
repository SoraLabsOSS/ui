...
bun run registry:build        # rebuild component registry (apps/www) — ALWAYS run `bun x ultracite fix` after this!
```
...
bun run registry:build        # merges registry-item.json files, builds public/r/*.json (follow with `bun x ultracite fix`)
```
...
There is no test runner configured in this repo — verification is via `check-types`, `lint`, and `registry:build`. Whenever `registry:build` is run, always execute `bun x ultracite fix` (or `bun run format:write`) to format and lint-fix all files generated by it.
...
- **After Registry Build**: Whenever `registry:build` is executed, you MUST run `bun x ultracite fix` (or `bun run format:write`) to automatically format and lint-fix all files generated by the registry generator (e.g. `apps/www/__registry__/*`, `public/r/*.json`).
