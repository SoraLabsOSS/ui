...
function formatDetail(item: RegistryItem): string {
  ...
  return [
    ...
    "",
    "## Install",
    `Run this in the user's project (not this MCP server, which has no access to their files): \`npx @soralabsoss/sora-cli add ${item.name} --yes\` — fetches this component with the correct import aliases for their project and installs its dependencies. Prefer this over copying the source below by hand.`,
...
class SoraGetComponentInfoTool extends MCPTool<GetComponentInfoInput> {
  ...
  description =
    "Get Sora UI component/hook info: with `name`, returns install guidance (recommending the sora-cli command), dependencies, and full source for reference; without `name`, lists all installable components and hooks.";
