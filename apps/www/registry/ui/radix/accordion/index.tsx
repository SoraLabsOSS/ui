...
/**
 * Animated Accordion component built on Radix UI with Motion spring physics,
...
function Accordion({
  ...
  const handleSingleChange = useCallback(
    (nextValue: string) => {
      setOpenValues(nextValue ? [nextValue] : []);
    },
    [setOpenValues]
  );
  ...
  const rootElement = multiple ? (
    ...
    <AccordionPrimitive.Root
      ...
      type="single"
      value={openValues[0] == null ? "" : String(openValues[0])}
      {...props}
    >
      {children}
    </AccordionPrimitive.Root>
...
interface AccordionItemProps
  extends Omit<ComponentProps<typeof AccordionPrimitive.Item>, "value"> {
  /**
  ...
  return (
    <AccordionHeader>
      ...
      >
        <motion.button
          type="button"
          variants={shouldAnimate ? { pressed: { scale: 0.99 } } : undefined}
