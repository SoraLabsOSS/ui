...
export const previewComponents: Record<string, any> = {`;
  ...
  const appendIndexEntry = (
    ...
    if (filesWithContent.length > 0) {
      sourcesToWrite.push({
        name: item.name,
        code: filesWithContent[0]?.content ?? "",
        files: filesWithContent,
      });
      sourcesLoadersList.push(
        `  "${item.name}": () => import("./sources/${item.name}.json"),`
      );
    }

    const filesMetadata = filesWithContent.map(({ path, type, target }) => ({
      path,
      type,
      target,
    }));
  ...
  "${item.name}": {
    ...
    files: ${JSON.stringify(filesMetadata, null, 2)},
    hasSource: ${filesWithContent.length > 0},
...
export { loadComponentSource, loadComponentFiles } from "./sources";`;
  ...
  for (const s of sourcesToWrite) {
    await fs.writeFile(
      path.join(sourcesDir, `${s.name}.json`),
      JSON.stringify(s, null, 2)
    );
  }

  const sourcesTs = `/** biome-ignore-all lint: Autogenerated file */
/* eslint-disable @typescript-eslint/no-explicit-any */
// @ts-nocheck
// This file is autogenerated by scripts/build-registry.mts
// Do not edit this file directly.
...
${sourcesLoadersList.join("\n")}
};

const sourceCache = new Map<string, string>();
const filesCache = new Map<string, any[]>();

export async function loadComponentSource(name: string): Promise<string | null> {
  ...
  return new Promise((resolve, reject) => {
    const child = spawn(
      ...
      [
        ...
        "__registry__/sources.ts",
        "__registry__/sources",
        "public/r",
      ],
      { cwd: process.cwd(), stdio: "inherit" }
    );
