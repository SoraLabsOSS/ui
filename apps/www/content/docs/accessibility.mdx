...
A few primitives (`scroll-gallery`, `text-reveal-block`, `text-reveal-box`) are driven by [GSAP](https://gsap.com/) instead of Motion — `MotionConfig` has no effect on them, since GSAP has no equivalent built-in setting.
