...
A few primitives (`scroll-gallery`, `sticky-scroll-cards`, `text-reveal-block`, `text-reveal-box`, `magnetic-cards`) are driven by [GSAP](https://gsap.com/) instead of Motion — `MotionConfig` has no effect on them, since GSAP has no equivalent built-in setting.
