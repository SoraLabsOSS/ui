...
Each logo requires an `alt` text via the `alt` prop. The root element should receive an `aria-label` describing the section (e.g., `"Partner logos"`). The swap animation cycles automatically with no play/pause control. When `prefers-reduced-motion: reduce` is set, consider setting a very long `interval` or disabling the component to avoid looping motion.
