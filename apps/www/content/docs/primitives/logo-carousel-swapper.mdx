...
Each logo requires an `alt` text via the `alt` prop. The root element should receive an `aria-label` describing the section (e.g., `"Partner logos"`). When `prefers-reduced-motion: reduce` is set, the row-swap interval doesn't start — the carousel freezes on the first row instead of cycling, with no extra configuration needed.
