...
| `get_component_info` | Without a `name`, lists installable Sora UI components and hooks with short descriptions. With a `name`, returns install guidance (the `sora-cli add`/`diff` commands to run), dependencies, file targets, and the full source for reference |
