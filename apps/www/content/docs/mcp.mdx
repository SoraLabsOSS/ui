...
| `get_component_info` | Without a `name`, lists installable Sora UI components and hooks (`registry:ui`, `registry:hook`). With a `name`, returns install guidance (`npx shadcn@latest add @soralabs/<name> --yes` or `sora-cli add`), dependency trees, file targets, and published agent metadata (accessibility constraints, composition rules, motion engine) — pass `includeSource: true` to also get the full source, and `cwd` for monorepos |
| `validate_composition` | Validate a proposed set of registry components against dependencies and published agent metadata before generating UI code. Reports missing items, registry dependency warnings, accessibility constraints, motion requirements, and composition rules |
...
| Server | Role |
| --- | --- |
| **`sora-ui`** | Read docs, browse sections/resources, inspect component info, and validate compositions — `search_docs`, `get_page`, `list_sections`, `get_component_info`, `validate_composition`, `install-component` prompt (also returns `shadcn add` / `sora-cli` install commands) |
| **`shadcn`** | Install `@soralabs/*` into your project via the shadcn CLI, as an agent-callable tool instead of a shell command |

Use **both** if you'd rather the agent call an MCP tool to install than run `shadcn`/`sora-cli` itself — register `@soralabs` in `components.json` first, see [Installation](/docs/installation). Most agents with shell access can just run the command `get_component_info` returns.
...
3. Agent calls sora-ui MCP → reads docs / gets component info → runs
   `npx shadcn@latest add @soralabs/scroll-gallery --yes` (or `sora-cli add`) → writes code in your repo
```
...
Cursor supports [one-click MCP install links](https://cursor.com/docs/mcp/install-links). Click below — Cursor opens a prompt to add the remote server, then enable it under **Settings → Tools & MCP**.
...
<CodeTabs
  ...
  "mcpServers": {
    "sora-ui": {
      "command": "node",
      "args": ["<path-to-repo>/apps/xmcp/dist/stdio.js"]
    }
  }
}`,
  }}
...
[Claude Code](https://claude.ai/code) is Anthropic's official CLI. You can connect via remote HTTP or local STDIO:

<CodeTabs
  lang="bash"
  codes={{
    'Remote HTTP': `claude mcp add --transport http sora-ui ${SORA_UI_MCP_URL}`,
    'Local STDIO': `claude mcp add sora-ui node <path-to-repo>/apps/xmcp/dist/stdio.js`,
  }}
/>
...
<CodeTabs
  ...
  "mcpServers": {
    "sora-ui": {
      "command": "npx",
      "args": ["-y", "mcp-remote", "${SORA_UI_MCP_URL}"]
    }
  }
}`,
    'Native HTTP': `{
...
cd apps/xmcp
bun install
bun run build       # compiles dist/stdio.js and dist/http.js
```

- **Run STDIO locally**: `node apps/xmcp/dist/stdio.js`
- **Run HTTP server locally**: `bun run start` (serves at `http://localhost:1337/mcp`)
- **Development with hot reload**: `bun run dev`
...
- Get the UI kit landing page (`ui`)
- List the UI section in Sora UI docs
...
- Validate composing `base-dialog`, `base-button`, and `base-tooltip`
- Check if `dialog` and `stagger-button` have any composition conflicts or accessibility constraints
- Validate my selected components before building a modal form: `base-dialog`, `base-checkbox`, `base-button`
