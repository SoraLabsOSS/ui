...
| `get_component_code` | Get the full source code for a component or hook by name, ready to paste in without running the install CLI |

<Callout>
  `list_components`, `get_component_info`, and `get_component_code` let you browse, inspect, and pull the source of a component directly from **Sora UI docs MCP** — handy if you just want to look something up or copy code by hand. To actually **install** `@soralabs/*` components with natural language, still configure the [shadcn MCP server](https://ui.shadcn.com/docs/mcp) in the same project (see [Registry MCP (shadcn)](#registry-mcp-shadcn) below).
</Callout>
...
| Server | Role |
| --- | --- |
| **`sora-ui`** | Read docs and browse the catalog — `search_docs`, `get_page`, `list_components`, `get_component_info`, `get_component_code` |
| **`shadcn`** | Install `@soralabs/*` into your project via the shadcn CLI |

Use **both** when you want the agent to look up usage *and* run `shadcn add` for a registry item. Register `@soralabs` in `components.json` first — see [Installation](/docs/installation).
...
```text
1. Enable sora-ui MCP (+ shadcn MCP if installing)
2. Register @soralabs in components.json
3. In Agent chat, ask naturally or explicitly:
   - "What Sora component fits a pinned scroll text reveal?"
   - "Install scroll-gallery and show usage with variant=studio"
4. Agent calls MCP → reads docs → (optionally) runs shadcn add → writes code in your repo
```
