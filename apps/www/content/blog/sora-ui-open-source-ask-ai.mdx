...
The Sora UI source is on GitHub: [`github.com/SoraLabsOSS/ui`](https://github.com/SoraLabsOSS/ui). Licensed under the MIT License (with upstream portions under MIT + Commons Clause). And the site has an **Ask AI** button that actually reads the docs before it talks — Cloudflare AI Search retrieves the matching pages, then a model answers from those excerpts.
...
- **Use it in client work.** That's the point. Free for personal, commercial, and client projects (only restricting standalone reselling of raw upstream components as a competing kit).
- **Bugs and "how do I" go to the [GitHub repo](https://github.com/SoraLabsOSS/ui).** The repository issues and discussions are open for community questions and bug reports.
...
3. If the answer cites the wrong page, that's useful. File it on the [GitHub repo](https://github.com/SoraLabsOSS/ui). The index is only as good as the crawl.
