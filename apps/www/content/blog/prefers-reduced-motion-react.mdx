...
description: What 24 shipped React primitives taught me about handling prefers-reduced-motion properly — five different strategies, and why "just set duration to 0" is the wrong default.
ogSubtitle: Five real strategies for prefers-reduced-motion, pulled from 24 production components.
author: Axyl
date: 2026-07-11
hashtags: ["#a11y", "#accessibility", "#css", "#react"]
---

I maintain [Sora UI](https://ui.soralabs.studio) — a small React component library: animated cards, custom cursors, scroll effects, the stuff people reach for to make a landing page feel alive. It's grown to 24 published motion primitives, all built on either GSAP or Motion, and both of those libraries will happily animate someone straight into a migraine if you let them.
...
useGSAP(() => {
  ...
  // ...mountCards, which is the only place applyReducedMotionEndState gets called
});
```
...
Every component discussed here — `StickyScrollCards`, `MagneticCards`, `Highlight`, `ProgressiveBlur`, `CustomCursor`, `Skeleton`, and the shared `usePrefersReducedMotion` hook itself — is open source in the [Sora UI](https://ui.soralabs.studio) registry, `npx shadcn add`-able, source included, no package to trust blindly.
