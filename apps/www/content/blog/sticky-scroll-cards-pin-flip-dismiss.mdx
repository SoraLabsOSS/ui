...
The `useGSAP` effect only bails early when there are no cards to mount — reduced motion is *not* a reason to skip the effect. It still mounts, still measures the scroller, still sets the cards' initial transforms — it just never creates a `ScrollTrigger`. That distinction matters: the effect has to keep running for a reduced-motion user, because — and this is the part that's easy to get lazy about — the honest reduced-motion behavior isn't *stop animating*, it's explicitly setting the **end state**:
