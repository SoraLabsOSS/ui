...
**[Cursor Trail Reveal](/catalog/cursor-trail-reveal)** copies the full source into `components/sora-ui/catalog/`. The trail logic, clip-path wipes, and reduced-motion guard are all sitting in files you own. Want a slower stagger or a different breakpoint? Change the timeline — no fork, no `!important` war with a package that doesn't know your layout exists.

**[Text Reveal Box](/catalog/text-reveal-box)** is scroll-pinned copy driven by GSAP ScrollTrigger. The tall track, sticky viewport, and word scrub all live in one place, in plain sight. Swap the easing. Shorten `pinDuration`. Rip GSAP out entirely if your stack moves to Motion next year — it's your code now, do what you want with it.
