...
<video autoPlay className="w-full rounded-md border border-border/60" loop muted playsInline src="https://cdn.soralabs.studio/media/demo/magnetic-cards/magetic-card.webm" />
...
Full source is at [/docs/motion/magnetic-cards](/docs/motion/magnetic-cards) — `npx shadcn add` it, it's a plain file in your repo. If the fling feels too aggressive or too tame for your images, `velocityMultiplier` and `rotationVelocityMultiplier` are two number props, not a rebuild.
